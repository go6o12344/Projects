...
class Player:
	...
	#Health
	health = 10
	#Reload timer. If below 15, increases when you move
	reloadTimer = 15
	#Hitbox
	dimX = 0.15
	dimy = 0.05
	...
	def move(self, map):
		...
		#Reset direction
		self.direction = [0,0]
		if self.reloadTimer<15:
			self.reloadTimer+=1
	def __init__(self):
		global mobs
		mobs.append(self)

	#This method name is oddly ironic
	def shoot(self):
		if reloadTimer<15:
			return
		global mobs
		max = 0
		precision = 192
		#We get the angle at which the bullet is shot, its sine and its cosine
		currAngle = self.AOV - FOV/2
		sinA = math.sin(currAngle)
		cosA = math.cos(currAngle)
		while 1:
			if map[math.floor(self.y+3*max*sinA/precision)][math.floor(self.x+3*max*cosA/precision)]==1:
				break
			max+=1
		for i in range(1,max+1):
			for mob in mobs:
				if mob!= self and (mob.y-mob.dimy<math.floor(self.y+3*max*sinA/precision)<mob.y+mob.dimy) and (mob.x-mob.dimx<math.floor(self.x+3*max*cosA/precision)<mob.x+mob.dimx):
					mob.health-=1


class NPC:
	#Current coordinates for the NPC
	x = 1.5
	y = 1.5
	health = 2
	reloadTimer = 15
	dimX = 0.15
	dimy = 0.05
	#Direction at which the NPC is looking initially. This is the angle between the leftmost part of the FOV and the X-axis. The player is technically looking at an angle of 0.
	AOV = FOV/2
	
	#Rotate method for NPC
	def rotate(self,dir):
		self.AOV+= dir * pi*sensitivity
	...
	#Move method
	def move(self, map):
		#I have to rewrite this from scratch for NPCs. For now I dont need NPCs to move so its fine
		if self.reloadTimer<15:
			self.reloadTimer+=1
	def __init__(self):
		global mobs
...
def drawNPC(player, mob, distance):
	pass
def sees(player, npc):
	x1 = npc.x
	y1 = npc.y
	...
	x = player.x
	y = player.y
	topRight = atan2(x1-X1-x,y1+Y1-y)
	topLeft = atan2(x1+X1-x,y1+Y1-y)
	botRight = atan2(x1-X1-x,y1-Y1-y)
	botLeft = atan2(x1+X1-x,y1-Y1-y)
	all = [topLeft, topRight, botLeft, botRight]
	for i in all:
		if player.AOV<i<player.AOV+FOV:
			return True		
	return False
def drawNPCs(mobs):
	visibleMobs = []
	player = mobs[0]
	distance = sqrt((player.x-npc.x)**2+(player.y-npc.y)**2)
	for npc in mobs[1:]:
		if distance<=3 and sees(player,npc):
			drawNPC(player,npc)
